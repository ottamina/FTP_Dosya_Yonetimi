...
Uygulamanın kompozisyon köküdür. Servisleri dependency injection kabına kaydeder, CORS politikasını kurar ve controller rotalarını açar.

```mermaid
flowchart LR
    Program["Program.cs"] --> Controllers["AddControllers"]
    Program --> DI["Servis kayıtları"]
    Program --> Cors["localhost:5173 CORS"]
    Program --> Routes["MapControllers"]
    Routes --> Run["app.Run"]
```
...
| `OpenSshSftpProvisioner` | Singleton | OpenSSH ve Windows hesap işlemleri merkezi yürür |
...
- `FtpManager.Api.csproj`: .NET 10 Windows hedefi ve FluentFTP, LiteDB, SSH.NET bağımlılıkları.
...
- Yönetici modunda güvenli ProgramData deposunu; normal modda eski yerel deposu seçmek.
...
- Güvenli kökü `C:/ProgramData/FtpManager/ftp_root` olarak hesaplar.
...
```mermaid
flowchart TD
    Start["Provision"] --> Admin["Windows + yönetici kontrolü"]
    Admin --> User["Windows hesabını oluştur/güncelle"]
    User --> ACL["NTFS ACL kur"]
    ACL --> Config["sshd_config Match User bloğu"]
    Config --> Backup["Yedek al"]
    Backup --> Test["sshd -t"]
    Test --> Restart["sshd servisini yeniden başlat"]
...
- `ConfigureDirectoryPermissions`: Chroot atalarında yazmayı kapatır; SFTP kullanıcısına geçiş/okuma, `data` içine Modify verir.
...
- `Deprovision`: Match bloğunu ve Windows hesabını kaldırır.
...
5. API adresleri frontend kodunda sabittir; ortam değişkenine taşınabilir.
6. `App.jsx` çok büyüktür; auth, explorer, upload ve server yönetimi custom hook'lara ayrılabilir.
7. FTP pasif port aralığı sabittir; firewall kurulumu ve ortam ayarıyla yapılandırılabilir olmalıdır.
